...
public class FileRepository implements IFileRepository {
	...
	public void uploadFile(FileVO file) {
		String sql = "insert into files (file_id, directory_name, file_name, file_size,file_content_type, "
				+ "file_upload_date, file_data) "
				+ "values(?,?,?,?,?,sysdate,?)";
				jdbctemplate.update(sql,file.getFileId(),file.getDirectoryName(),file.
				getFileName(),
	...
	public FileVO getFile(int fileId) {
		...
		{
			@Override
			public FileVO mapRow(ResultSet rs, int rowNum) throws
			SQLException {
			FileVO file = new FileVO();
			file.setFileId(rs.getInt("file_Id"));
			file.setDirectoryName(rs.getString("directory_name"));
